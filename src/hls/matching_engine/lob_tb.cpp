...
static bool test_add_bid_updates_best() {
    ...
    // Price: $175.30 = 1753000; within window [BASE=1750000, BASE+63*100]
    OrderMsg msg{};
    msg.msg_type  = MSG_ADD;
    msg.order_ref = 1ULL;
    msg.side      = 0;          // bid
    msg.shares    = 500;
    msg.price     = 1753000;    // $175.30

    BookUpdate upd = send_msg(msg);

    CHECK(upd.best_bid == 1753000, "best_bid should equal added price");
...
static bool test_spread_and_mid() {
    ...
    bid.shares = 200; bid.price = 1752000; // $175.20
    send_msg(bid);

    // Ask at $175.40
    OrderMsg ask{};
    ask.msg_type = MSG_ADD; ask.order_ref = 11; ask.side = 1;
    ask.shares = 150; ask.price = 1754000; // $175.40
    BookUpdate upd = send_msg(ask);

    CHECK(upd.best_bid == 1752000, "best_bid mismatch");
    CHECK(upd.best_ask == 1754000, "best_ask mismatch");
    CHECK(upd.spread   == 2000,    "spread should be $0.20 = 2000");
    CHECK(upd.mid_price == 1753000, "mid should be $175.30");
    return true;
}
...
static bool test_delete_clears_best() {
    ...
    add.shares = 100; add.price = 1751000;
    send_msg(add);
...
static bool test_partial_execute() {
    ...
    add.shares = 400; add.price = 1755000;
    send_msg(add);
    ...
    CHECK(upd.best_ask == 1755000, "ask should still be present after partial execute");
...
static bool test_vs_golden_model() {
    ...
    TestMsg msgs[] = {
        {'A', 'B', 100, 300, 1752000, 0, 0, 0},  // add bid $175.20
        {'A', 'S', 101, 200, 1754000, 0, 0, 0},  // add ask $175.40
        {'A', 'B', 102, 150, 1751000, 0, 0, 0},  // add bid $175.10
...
// =========================================================================
// TEST 6: Out-of-window price → add is dropped, book state unaffected
//
// The flat book covers [BASE_PRICE, BASE_PRICE + 63 ticks]. Prices outside
// that window must be ignored (production would re-center the window) —
// and must never corrupt in-window state.
// =========================================================================
static bool test_out_of_window_ignored() {
    reset_book();

    // In-window bid at $175.20
    OrderMsg in{};
    in.msg_type = MSG_ADD; in.order_ref = 40; in.side = 0;
    in.shares = 100; in.price = 1752000;
    send_msg(in);

    // Out-of-window bid at $182.50 — must be dropped
    OrderMsg out_high{};
    out_high.msg_type = MSG_ADD; out_high.order_ref = 41; out_high.side = 0;
    out_high.shares = 999; out_high.price = 1825000;
    BookUpdate upd = send_msg(out_high);
    CHECK(upd.best_bid == 1752000, "out-of-window add must not change best_bid");

    // Below-window bid at $170.00 — must also be dropped
    OrderMsg out_low{};
    out_low.msg_type = MSG_ADD; out_low.order_ref = 42; out_low.side = 0;
    out_low.shares = 999; out_low.price = 1700000;
    upd = send_msg(out_low);
    CHECK(upd.best_bid == 1752000, "below-window add must not change best_bid");
    CHECK(upd.best_ask == 0, "ask side must remain empty");
    return true;
}
...
int main() {
    ...
    run_test("Out-of-window price is safely dropped", test_out_of_window_ignored);
